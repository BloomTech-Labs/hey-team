...
class Participants extends React.Component {
  ...
  handleDisplayCards() {
    for (
      let i = 0;
      i < conversationsArray[conversationsArrayPosition].participants[0].length;
      ++i
    ) {
      this.setState({ displayArray: [] });
      cardArray[i] = (
        <div key={v4()} className="participants__userImage">
          <button
            name={i}
          ...
          <img
            src={
              conversationsArray[conversationsArrayPosition].participants[0][i]
                .imageData
            }
            alt="User Photo"
          />
  ...
  handleDelete(e) {
    conversationsArray[conversationsArrayPosition].participants[0].splice(
  ...
  render() {
    return (
      <div className="participants__flexWrapper">{this.state.displayArray}</div>
    );
  }
}
